...
@Component({
  ...
  template: `
    <div class="flex h-full flex-col gap-[16px] ">
      ...
      <div class="sm2:items-start flex items-center flex-col gap-4 sm2:place-content-center  sm2:grid sm2:grid-rows-3   sm2:grid-flow-col ">
        <div
          class=" w-full h-fit max-w-[429px] rounded-[3px] border border-base-content/50"
        >
        ...
        <div
          class=" h-full row-span-2 flex w-full max-w-[429px] flex-col justify-start rounded-[3px] border border-base-content/50"
        >
          <div class="bg-primary p-4 text-primary-content">
          ...
          <p class="text-base text-base-content/70">The display value of a grid item is blockified: if the specified display of an in-flow child of an element generating a grid container is an inline-level value, it computes to its block-level equivalent</p>
          </div>
        </div>
        <div
          class=" w-full h-fit max-w-[429px] rounded-[3px] border border-base-content/50"
...
export class DashboardComponent {
  ...
  accomplishedMilestones: ChartData<'pie', number[], string | string[]> = {
    ...
    datasets: [
      { 
        data: [3, 5],  backgroundColor: ['#0b874b', '#3127b4'],
      },
    ],
  };
  totalTaskAssigmentData: ChartData<'pie', number[], string | string[]> = {
    ...
    datasets: [
      {
        data: [23, 50], backgroundColor: ['#0b874b', '#3127b4'],
      },
    ],
  };
